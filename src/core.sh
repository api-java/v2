...
change() {
    ...
    11)
        ...
        [[ $(grep -i "^beevpn.cn$" <<<$is_servername) ]] && {
            err "你干嘛～哎呦～"
        }
        add $net
        ;;
...
info() {
    ...
    ss)
        ...
        is_url="ss://$(echo -n ${ss_method}:${ss_password} | base64 -w 0)@${is_addr}:${port}#v2-$net-${is_addr}"
    ...
    ws | h2 | grpc)
        ...
        [[ $is_protocol == 'vmess' ]] && {
            ...
            is_url="$is_protocol://$uuid@$host:$tlsport?encryption=none&security=tls&type=$net&host=$host&${is_url_path}=$(sed 's#/#%2F#g' <<<$path)#v2-$net-$host"
        }
    ...
    reality)
        ...
        is_url="$is_protocol://$uuid@$ip:$port?encryption=none&security=reality&flow=xtls-rprx-vision&type=tcp&sni=$is_servername&pbk=$is_public_key&fp=ios#v2-$net-$is_addr"
        ;;
    door)
